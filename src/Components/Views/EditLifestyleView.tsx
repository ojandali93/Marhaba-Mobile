...
const EditLifestyleView = () => {
  ...
  const [travel, setTravel] = useState(profile?.Lifestyle[0].travel || '');
  const [social, setSocial] = useState(profile?.Lifestyle[0].social || '');
  const [health, setHealth] = useState(profile?.Lifestyle[0].health || '');
  const [finances, setFinances] = useState(
    profile?.Lifestyle[0].finances || '',
  );
  const [living, setLiving] = useState(profile?.Lifestyle[0].living || '');
  ...
  const loadCoreViews = () => {
    setTravel(profile?.Lifestyle[0].travel);
    setSocial(profile?.Lifestyle[0].social);
    setHealth(profile?.Lifestyle[0].health);
    setFinances(profile?.Lifestyle[0].finances);
    setLiving(profile?.Lifestyle[0].living);
  };
