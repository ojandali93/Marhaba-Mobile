...
const EitherOrEditView = () => {
  ...
  const questionStateMap = {
    ...
    'Do you believe in love at first sight?': [firstSight, setFirstSight, 'firstSight'],
    ...
    'Long-distance relationships can work?': [longDistance, setLongDistance, 'longDistance'],
  };
  ...
  const countAnswered = () => {
    return Object.values(questionStateMap).filter(([val]) => val && val.trim() !== '').length;
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put('https://marhaba-server.onrender.com/api/account/updateSurvey', {
  ...
  return (
    <View>
      ...
      <Text style={tailwind`text-base font-semibold text-gray-800`}>Either / Or</Text>
          {expanded ? (
            changeDetected && countAnswered() >= 6 ? (
              <TouchableOpacity onPress={handleUpdate}>
                <Text style={[tailwind`text-base font-bold px-2 py-1 rounded-md text-white`, { backgroundColor: themeColors.primary }]}>Save</Text>
              </TouchableOpacity>
            ) : (
      ...
      {expanded && (
        <ScrollView style={[tailwind`mt-4 px-2`, { marginTop: screenHeight * 0.02 }]}>
          {eitherOrQuestions.map((q, index) => {
  const [value, setFunc, key] = questionStateMap[q.question];
  return (
    <View key={index} style={tailwind`mb-4`}>
      <Text style={tailwind`text-lg font-semibold mb-2`}>{q.question}</Text>
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                ...
                isSelected ? tailwind`bg-[#008080] border-[#008080]` : tailwind`border-gray-400`,
              ]}
            >
              <Text
                style={[
                  tailwind`text-base font-semibold`,
                  isSelected ? tailwind`text-white` : tailwind`text-black`,
                ]}
              >
                {option}
              </Text>
